...
echo ---------- Building the Simulated Device Cloud Upload IoT Edge sample ----------
gcc src/main.c src/module_service_config_disabled.c --std=c99 -I/usr/include/azureiot -I/usr/include/azureiotedge -I"$build_root"/inc -L. -lgateway -laziotsharedutil -o "$build_dir"/"$output_name"
[ $? -eq 0 ] || exit $?
